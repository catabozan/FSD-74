...
export const createMovie = (req, res) => {
    ...
    if (
        ...
        || movieData.description.length > 10000
    ) {
        res.json({
            error: "the movie description is invalid"
        })
    ...
    if (
        ...
        res.json({
            error: "the movie rating is invalid"
        })
    ...
    // movies.push({ ...movieData, id: id })

    movies.push(movieData)

    res.json(
        formatResponse(movieData)
    )
}
